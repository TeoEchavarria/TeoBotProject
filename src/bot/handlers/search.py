...
async def search_embeddings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context_emb, answer = (
            await response_flow(update.message.text, user["openai_key"])
        ...
        answer = {
            "text": "SEARCH: I'm sorry I couldn't generate an answer for you. Would you like to ask me something else?"
        }
    keyboard = [
        [
            InlineKeyboardButton(
                note["url"].replace("-", " "),
                url=f"{os.getenv('WEB_NOTES')}{note['url']}",
            )
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=answer["text"], reply_markup=reply_markup
    )
